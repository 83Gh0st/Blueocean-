...
import { Package, FlaskConical, Microscope, Tag, Warehouse, Truck, Factory } from "lucide-react";
...
// The same seven stops, flattened for the mobile stepper — one source of
// truth (NODES) so the two layouts can't quietly drift out of sync.
const STEPS = [
  ...NODES.map(({ label, sub, icon }) => ({ label, sub, icon, final: false })),
  { label: "Your Plant", sub: "Dosed & in service", icon: Factory, final: true },
];
  ...
  return (
    <section className="process" id="process">
      ...
      <div className="wrap">
        ...
        {/* The ref lives on this shared wrapper (not on either child) so
            useInView keeps working no matter which of the two layouts
            below is the one actually visible at the current width. */}
        <div className="flow-wrap" ref={ref}>
          {/* Tablet & up — the animated S-curve. Untouched. */}
          ...
          {/* Mobile — a vertical stepper. Every label is real HTML text sized
              in rem, so it reads cleanly at any phone width instead of
              shrinking away with an SVG viewBox. */}
          <ol className="flow-steps">
            {STEPS.map((step, i) => {
              ...
              return (
                <motion.li
                  key={step.label}
                  className={`flow-step${step.final ? " flow-step-final" : ""}`}
                  initial={{ opacity: 0, x: -16 }}
                  animate={inView ? { opacity: 1, x: 0 } : {}}
                  transition={{ duration: 0.5, delay: 0.1 + i * 0.09, ease: [0.16, 1, 0.3, 1] }}
                >
                  <span className="flow-step-rail" aria-hidden="true">
